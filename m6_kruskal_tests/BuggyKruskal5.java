...
public class Kruskal {
	...
	public static void main(String[] args) {
	    ...
	    boolean done = false; 
	    Edge toRemove = null; 
	    for (Edge e : result.edges()) if (!done)
		if (e.getSrcEdges().size() >= 2 && e.getDstEdges().size() >= 2) {
	    		src = e.getSrc().getCity(); 
	    		dst = e.getDst().getCity(); 
	    		weight = e.getWeight(); 
	    		toRemove = e; 
			done = true; 
		}
	    if (done) {
		    result.removeEdge(toRemove);
		    Vertex v1 = null;
		    Vertex v2 = null; 
		    for (Vertex v : result.vertices()) if (v1 == null || v2 == null)
			if (v.getCity() != src && v.getCity() != dst)
			    if (v1 == null) v1 = v; 
			    else if (v2 == null) v2 = v; 
		    if (v1 != null && v2 != null)
			result.insertEdge(v1, v2, weight);
	    }
	    
	    try {
		    PrintWriter pw = new PrintWriter(new FileWriter(args[1]));//("src/data/cities_sol.txt"));
		    for (int i = 0 ; i < result.edges().size(); i++) {
		    	pw.println(result.edges().get(i).getSrc().getCity() + "\t" + result.edges().get(i).getDst().getCity() + "\t" + result.edges().get(i).getWeight());
		    }
			pw.close();
	    } catch (IOException e) {
